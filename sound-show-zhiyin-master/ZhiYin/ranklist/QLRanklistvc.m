...
#import "QLTianyavc.h"
#import "QLcomplaintvc.h"
#import "BlacklistM+CoreDataClass.h"

@interface QLRanklistvc ()
{
...
-(void)loadtianya:(id)btn {
    [self showEmptyViewWithLoading:YES image:nil text:@"风云榜正在加载的路上..." detailText:nil buttonTitle:nil buttonAction:nil];
...
- (void)refresh_support {
    if (_supportdirty) {
        ...
        [QLglobalvar shareglobalvar].ranklist_support_dirty = _supportdirty;
        _supportdirty = NO;
    }
}
...
- (void)settableview {
    ...
    [header setTitle:@"风云榜语音正在路上..." forState:MJRefreshStateRefreshing];
    ...
    [footer setTitle:@"风云榜语音正在路上..." forState:MJRefreshStateRefreshing];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        QLRanklistCell* cell = [QLRanklistCell cellwithTableview:tableView];
        protocol_audio_info* audioinfo = self.audiolist[indexPath.row];
        cell.nickname.text = audioinfo.nickname;
        cell.nickname.textColor = MAIN_BLUE_COLOR;
...
-(void)complain_action:(UIButton*)complainbtn {
    ...
    if (ip) {
        ...
        if ([audioinfo.clientid isEqualToString:[QLglobalvar shareglobalvar].clientID]) {
            [QMUITips showInfo:@"请不要投诉自己，对自己好一点，不要发布不良内容就可以了。" inView:self.view hideAfterDelay:3];
            return;
        }
        QLcomplaintvc* cvc = [[UIStoryboard storyboardWithName:@"complaint" bundle:nil]instantiateViewControllerWithIdentifier:@"complaint"];
        [cvc complaint_who:audioinfo.clientid nickname:audioinfo.nickname audioid:audioinfo.audioid onvcindex:3];
        [[QLglobalvar shareglobalvar].tabbarcontroller changevc_overrank:cvc title:@"寡人怒了"];
    }
}
...
-(void)isayaction:(UIButton*)isaybtn {
    ...
    if (ip) {
        ...
        [QLglobalvar shareglobalvar].towhere = 1;
        [QLglobalvar shareglobalvar].toclientid = audioinfo.clientid;
        [QLglobalvar shareglobalvar].tonickname = audioinfo.nickname;
        UITabBarController* rootvc = (UITabBarController*)[UIApplication sharedApplication].delegate.window.rootViewController;
        rootvc.selectedIndex = 1;
        [QLglobalvar shareglobalvar].current_tab = 1;
    }
}
...
- (void)reset_lastaudioimage {
    ...
    if (selindexpath) {
        QLRanklistCell* cell = (QLRanklistCell*)[self.ranklisttv cellForRowAtIndexPath:selindexpath];
...
-(void)viewDidAppear:(BOOL)animated {
    if ([QLglobalvar shareglobalvar].tianya_support_dirty) {
        NSArray* tianya_audiolist = [(QLTianyavc*)[QLglobalvar shareglobalvar].taiyavc tianya_audiolist];
        BOOL needfresh = NO;
        ...
        [QLglobalvar shareglobalvar].tianya_support_dirty = NO;
    }
