...
#import "zytabbar/QLZYTabBarController.h"
#import "utils/QLglobalvar.h"
...
#import "utils/QLcommom_utils.h"
#import "me/QLMeViewController.h"
...
- (BOOL)requestnickname {
    ...
    if (nicknameinfo.IsSuccess) {
        ...
        [QLglobalvar shareglobalvar].nickname = nicknameinfo.NickName;
        ret = YES;
    }
    else {
...
-(void)newmsg_uiflag {
    ...
    [tbvc.tabBar showLastBadgeOnItemIndex:4];
    QLMeViewController* mevc = (QLMeViewController*)[QLglobalvar shareglobalvar].mevc;
    ...
    if (sendme_audio.IsSuccess) {
        if ([sendme_audio.audio_info_list count] > 0) {
            ...
            if (item) {
                ...
                int more = [QLcommom_utils compareDate:local_lasttime withDate:msgtime];
                if (more != 1) {
                    newmsg = NO;
                }
            }
            if (newmsg) {
                [QLglobalvar shareglobalvar].newmsg_sendtome = YES;
...
- (void)createTabbar {
    ...
    UITabBarController* tabbarvc = [[QLZYTabBarController alloc]init];
...
//    UINavigationController* nav = [[UINavigationController alloc]initWithRootViewController:tabbarvc];
    ...
    [self.window makeKeyAndVisible];
    [QLglobalvar shareglobalvar].tabbarcontroller = (QLZYTabBarController*)tabbarvc;
