...
#import "QLzhiyiaboutvc.h"
#import "isendvc.h"
#import "QLsendmevc.h"
#import "QLblacklistvc.h"

@interface QLMeViewController ()

@end

@implementation QLMeViewController

-(void)setnickinfo {
    if ([QLglobalvar shareglobalvar].nickname) {
        self.nickinfo.text = @"您是一个有身份的人，您的大号是：";
        self.nickinfo.numberOfLines = 1;
        self.nickinfo.textColor = [UIColor grayColor];
        self.nickname.text = [QLglobalvar shareglobalvar].nickname;
        self.nickname.font = [UIFont systemFontOfSize:20];
        self.nickname.textColor = MAIN_BLUE_COLOR;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            NSString* cellid = [NSString stringWithFormat:@"cell%d_%d", (int)indexPath.section, (int)indexPath.row];
            QMUITableViewCell* cell = [[QMUITableViewCell alloc]initForTableView:tableView withStyle:UITableViewCellStyleDefault reuseIdentifier:cellid];
            if ([QLglobalvar shareglobalvar].newmsg_sendtome) {
...
-(void)newmsg_uiflag_hidden {
    UITabBarController* tbvc = [QLglobalvar shareglobalvar].tabbarcontroller;
    [tbvc.tabBar hideBadgeOnItemIndex:4];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            [[QLglobalvar shareglobalvar].tabbarcontroller changevc_overme:vc title:@"寡人语录"];
        }
        else if (indexPath.row == 1) {
            // send to me
            [QLglobalvar shareglobalvar].newmsg_sendtome = NO;
            [tableView reloadRowsAtIndexPaths:[NSArray arrayWithObjects:indexPath, nil] withRowAnimation:UITableViewRowAnimationNone];
            [self newmsg_uiflag_hidden];
            QLsendmevc* vc = [[UIStoryboard storyboardWithName:@"sendmevc" bundle:nil]instantiateViewControllerWithIdentifier:@"sendmevc"];
            [[QLglobalvar shareglobalvar].tabbarcontroller changevc_overme:vc title:@"有事启奏"];
        }
        else if (indexPath.row == 2) {
            // blacklist
            QLblacklistvc* vc = [[UIStoryboard storyboardWithName:@"blacklist" bundle:nil]instantiateViewControllerWithIdentifier:@"blacklist"];
            [[QLglobalvar shareglobalvar].tabbarcontroller changevc_overme:vc title:@"小黑屋"];
        }
        else if (indexPath.row == 3) {
            // zhiyi about
            QLzhiyiaboutvc* aboutvc = [[UIStoryboard storyboardWithName:@"zhiyi_about" bundle:nil]instantiateViewControllerWithIdentifier:@"zhiyi_about"];
            [[QLglobalvar shareglobalvar].tabbarcontroller changevc_overme:aboutvc title:@"关于知音"];
        }
    }
}
