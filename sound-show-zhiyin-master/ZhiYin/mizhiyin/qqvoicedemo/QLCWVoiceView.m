...
@interface QLCWVoiceView ()<UIScrollViewDelegate>
...
- (void)setupSelectLabel:(UILabel *)label {
    ...
    label.textColor = MAIN_BLUE_COLOR;
    _selectLabel = label;
}
...
- (CWChangeVoiceView *)voiceChangeView {
    if (_voiceChangeView == nil) {
        CWChangeVoiceView *voiceChangeView = [[CWChangeVoiceView alloc] initWithFrame:CGRectMake(self.cw_width * 1, 0, self.cw_width, self.contentScrollView.cw_height)];
...
//    talkBackLabel.textColor = MAIN_BLUE_COLOR;
...
- (void)setupSmallCircleView {
    ...
    view.backgroundColor = MAIN_BLUE_COLOR;
