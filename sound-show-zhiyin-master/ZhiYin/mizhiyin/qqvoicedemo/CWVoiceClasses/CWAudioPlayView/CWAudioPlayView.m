...
- (void)setupSendButtonAndCancelButton {
    CGFloat height = 40;
    UIButton *returnbtn = [self buttonWithFrame:CGRectMake(0, self.cw_height - height, self.cw_width / 2.0, height) title:@"放弃并返回" titleColor:MAIN_BLUE_COLOR font:[UIFont systemFontOfSize:18] backImageNor:@"aio_record_cancel_button" backImageHighled:@"aio_record_cancel_button_press" sel:@selector(btnClick:)];
...
- (BOOL)request_sendaudio {
    ...
    NSDictionary* param = [zyprotocol_sendaudio sendaudio_parame:audiodata audiolen:[self.stateView recordduration] towhere:[QLglobalvar shareglobalvar].towhere+1 otherid:[QLglobalvar shareglobalvar].toclientid];
    NSError *error = nil;
...
//    NSLog(@"%@",btn.titleLabel.text);
    ...
    if ([self.stateView recordduration] < minsec) {
        [QMUITips showInfo:@"可以再多说两句吗？请不要短于两秒" inView:[QLglobalvar shareglobalvar].mizhiyinvc.view hideAfterDelay:3];
        shouldsend = NO;
    }
    ...
    if (btn == self.sendButton && shouldsend) { // 发送
        // wav to aac
        NSString* tpath = [NSString stringWithFormat:@"%@.m4a", [CWRecordModel shareInstance].path];
        self.audioconverter = [[QLTPAACAudioConverter alloc]initWithDelegate:self source:[CWRecordModel shareInstance].path destination:tpath];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(ctx, [MAIN_BLUE_COLOR CGColor]);
