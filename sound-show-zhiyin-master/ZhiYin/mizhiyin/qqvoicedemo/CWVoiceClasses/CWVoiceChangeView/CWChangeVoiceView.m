...
- (CWVoiceChangePlayView *)playView {
    if (_playView == nil) {
        ...
        [(QLCWVoiceView *)self.superview.superview setState:CWVoiceStatePlay];
...
- (void)startRecorde:(UIButton *)btn {
    ...
    // 设置状态 隐藏小圆点和三个标签
    [(QLCWVoiceView *)self.superview.superview setState:CWVoiceStateRecord];
    ...
    [self animationMicBtn:^(BOOL finished) {
        ...
        dispatch_source_set_event_handler(_self->_record_timer, ^{
            ...
            if (recordtime >= LIMIT_RECORD_DURATION) {
                ...
                [(QLCWVoiceView *)self.superview.superview setState:CWVoiceStateDefault];
                self.playView = nil;
                [self playView];
            }
        });
...
- (void)endRecord:(UIButton *)btn {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(t * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (t == 0 && cost >= 2) {
            ...
            NSLog(@"录音时间太短");
            [QMUITips showInfo:@"真是惜字如金，可以再多唱两句吗？请不要短于两秒" inView:[QLglobalvar shareglobalvar].mizhiyinvc.view hideAfterDelay:3];
        }
    });
