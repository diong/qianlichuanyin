...
- (void)setRecordState:(CWRecordState)recordState {
    ...
    switch (recordState) {
        case CWRecordStateDefault:
            self.titleLb.text = @"点击录音（<-往左滑，伪装录音）";
            [self updateLableFrame:self.titleLb];
            break;
        ...
        case CWRecordStateTouchChangeVoice:
            self.titleLb.text = @"按住录音（未发送，松手可编辑）";
            [self updateLableFrame:self.titleLb];
            break;
...
- (void)preparePlay {
    ...
    for (int i = (int)(self.allLevels.count - 1) ; i >= (int)(self.allLevels.count - 10) ; i--) {
        CGFloat l = 0.05;
        if (i >= 0) {
